...
export class CarinhoProcessadorComponent implements OnInit {
  ...
  carregarProdutos(): void {
    this.carrinhoService.obterProdutos().subscribe((produtos) => {
      ...
      // Carregar imagens só uma vez para cada produto
      produtos.forEach((item) => {
        if (!this.imgUrls.has(item.id)) {
          this.processadorService.findById(item.id).subscribe({
            next: (processador) => {
              if (processador.imagens && processador.imagens.length > 0) {
                const imageUrl = this.processadorService.getImageUrl(
                  processador.imagens[0]
                );
                this.imgUrls.set(item.id, imageUrl);
              } else {
                this.imgUrls.set(item.id, '/img/processor-placeholder.png');
              }
            },
            error: () => {
              this.imgUrls.set(item.id, '/img/processor-placeholder.png');
  ...
  calcularTotais(): void {
    this.subtotal = this.produtosNoCarrinho.reduce(
  ...
  getImageUrl(id: number): string {
    const data = this.processadorService.findById(id);

    console.log(`Obtendo URL da imagem para o ID: ${id}`);

    console.log(this.processadorService.getImageUrl(id.toString()));

    return this.processadorService.getImageUrl(id.toString());
  }

  carregarImagem(item: Produto): void {
    this.processadorService.findById(item.id).subscribe({
      next: (processador) => {
        if (processador.imagens && processador.imagens.length > 0) {
          const imageUrl = this.processadorService.getImageUrl(
          ...
          console.log(`Imagem carregada para o produto ${item.id}: ${imageUrl}`);
        } else {
          console.warn(`Nenhuma imagem encontrada para o produto ${item.id}`);
        }
      },
      error: () => {
        console.error(`Erro ao carregar a imagem para o produto ${item.id}`);
      },
    });
  }
}
